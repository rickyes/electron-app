    ...
    request(list_url, function (error, response, body) {
      if (!error) {
        ...
        for(var i=0;i<body.length;i++){
          html += '<tr><td><a href="javascript:void(0);" data-id="'+body[i].idcard+'">'+body[i].name+'</a></td><td>'+body[i].idcard+'</td><td>'+body[i].sex+'</td><td>'+(body[i].result_info == null ? '未总结': body[i].result_info)+'</td><td>'+body[i].birthtime+'</td><td><img src="'+img_url+body[i].idcardimg+'" style="width:60px;height:60px;border-radius:50px;"/></td></tr>';
        }
